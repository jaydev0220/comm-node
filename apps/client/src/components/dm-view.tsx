...
interface DmViewProps {
	...
	onMessageDeleteIntent?: (intent: DmMessageDeleteIntent) => void;
}

export interface DmMessageDeleteIntent {
	messageId: string;
	deleteScope: 'sender';
}
...
const mapChatMessageToDmMessage = (message: ChatMessage): DmMessage => ({
	id: message.id,
...
const mapWsMessageToDmMessage = (message: WsMessage): DmMessage => ({
	id: message.id,
...
export function DmView({ accessToken, currentUser, friend, onMessageDeleteIntent }: DmViewProps) {
	...
	const findExistingDirectConversationId = useCallback(async (): Promise<string | null> => {
		...
		while (true) {
			...
			const directConversation = response.data.find((chat) => {
				if (chat.type !== 'DIRECT') {
					return false;
				}

				return chat.participants.some((participant) => participant.user.id === friend.id);
			});
	...
	const getOrCreateDirectConversationId = useCallback(async (): Promise<string> => {
		try {
			const createdConversation = await api.post<Chat>('/chats', {
				type: 'DIRECT',
			...
			if (!existingConversationId) {
				throw new Error('找不到既有私訊對話');
			}

			return existingConversationId;
		}
	}, [findExistingDirectConversationId, friend.id]);

	const loadInitialMessages = useCallback(async (nextConversationId: string): Promise<void> => {
		const searchParams = new URLSearchParams({
			limit: String(MESSAGES_PAGE_LIMIT)
		});
		const response = await api.get<MessageHistoryResponse>(
			`/chats/${nextConversationId}/messages?${searchParams.toString()}`
		);
		const initialMessages = response.data.map(mapChatMessageToDmMessage).reverse();
	...
	const loadOlderMessages = useCallback(async () => {
		...
		try {
			...
			const nextOlderMessages = response.data.map(mapChatMessageToDmMessage).reverse();
	...
	useEffect(() => {
		...
		const setupConversation = async () => {
			try {
				const nextConversationId = await getOrCreateDirectConversationId();
				...
				await loadInitialMessages(nextConversationId);
			} catch (error) {
				if (!isActive) {
					return;
				}
	...
	}, [friend.id, getOrCreateDirectConversationId, loadInitialMessages]);
		...
		const onMessage = (event: MessageEvent) => {
			...
			if (parsedMessage.event === 'message:new') {
				...
				setMessages((currentMessages) => {
					...
					if (payload.sender.id === currentUser.id) {
						...
						if (firstPendingRequestId) {
							...
							const withReplacement = currentMessages.map((message) => {
								...
								replaced = true;
								return mapWsMessageToDmMessage(payload);
							});
					...
					return [...currentMessages, mapWsMessageToDmMessage(payload)];
				});
				shouldScrollToBottomRef.current = true;
				return;
			}
	...
	const handleCopyMessage = useCallback(async (message: DmMessage): Promise<void> => {
	...
	const handleDeleteMessageIntent = useCallback(
		(message: DmMessage, actionState: MessageActionState) => {
			if (
				actionState.deleteScope !== 'sender' ||
	...
	return (
		<div className="bg-background flex h-full w-full flex-col overflow-hidden">
			<header className="border-border bg-surface flex items-center gap-2 border-b px-4 py-3">
				<div className="pointer-events-none">
					<Avatar name={friend.displayName} avatarUrl={friend.avatarUrl} size="sm" />
				</div>
				<div className="min-w-0">
					<p className="text-text-primary truncate text-sm font-semibold">{friend.displayName}</p>
				</div>
			</header>
				...
				>
					...
					{messages.map((message) => {
						...
						const actionState: MessageActionState = {
							canEdit: isSelf,
							deleteScope: isSelf ? 'sender' : 'none'
						};
